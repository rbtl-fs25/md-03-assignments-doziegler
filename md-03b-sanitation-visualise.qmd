...
```{r}
glimpse(sanitation)
# there are 8 variables in the ds
```
...
```{r}
sanitation_filt <- sanitation %>%
  filter(name == "Benin",
         year %in% c(2000,2020),
         varname_short != "san_sm"
         )
sanitation_filt_1 <- sanitation %>%
  filter(name == "Benin",
         year %in% c(2000,2020)
         )
# I don't quite understand the difference between year == c(2000,2020) and year %in% c(2000,2020)
```
...
1.  the legend is a bit confusing as no sanitation facilities is not on the bottom
2.  I would change the order from bottom to top: No, Unimproved, Limited, Basic Sanitation services
3.  It seems like in this category there are only NA values, also the percentages of the other categories seem to already add up to 100 %, so i don't quite understand this category
...
2.  What do you observe when you look at the code and plot?
    ...
    this plot shows the sanitation data over all countries of the world, with the the count of countries on the y axis and the percentage in the x axis. the plot is divided into the categories residence and types of sanitation available. The y axis is free which allows for an individual scaling of the y axis depending on the data range
